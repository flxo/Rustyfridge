...
mod test {
    ...
    impl Logger {
        fn plot(&self, file: &str) {
            let setpoint_adc_raw = self.process_data
                .iter()
                .map(|s| s.setpoint_adc_raw)
                .collect::<::std::vec::Vec<_>>();
            let current_adc_raw = self.process_data
                .iter()
                .map(|s| s.current_adc_raw as f32 / 10.0)
                .collect::<::std::vec::Vec<_>>();
            let setpoint_mdeg = self.process_data
                .iter()
                .map(|s| s.setpoint_mdeg as f32 / 1000.0)
                .collect::<::std::vec::Vec<_>>();
            let current_mdeg = self.process_data
                .iter()
                .map(|s| s.current_mdeg as f32 / 1000.0)
                .collect::<::std::vec::Vec<_>>();
            let setpoint_adc =
                self.process_data.iter().map(|s| s.setpoint_adc).collect::<::std::vec::Vec<_>>();
            let current_adc = self.process_data
                .iter()
                .map(|s| s.current_adc as f32 / 10.0)
                .collect::<::std::vec::Vec<_>>();
            let compressor = self.process_data
                .iter()
                .map(|s| match s.compressor {
                    false => 0.0,
                    true => 1.0,
                })
                .collect::<::std::vec::Vec<_>>();
            ...
            f.axes2d()
                ...
                .set_size(1.0, 1.0)
                .lines(0..current_mdeg.len(),
                       current_mdeg,
                       &[Caption("current [mdeg]"), Color("blue")])
                .lines(0..setpoint_mdeg.len(),
                       setpoint_mdeg,
                       &[Caption("setpoint [mdeg]"), Color("green")])
                .lines(0..current_adc.len(),
                       current_adc,
                       &[Caption("current"), Color("red")])
                .lines(0..setpoint_adc.len(),
                       setpoint_adc,
                       &[Caption("setpoint"), Color("cyan")])
                .lines(0..current_adc_raw.len(),
                       current_adc_raw,
                       &[Caption("current [raw]"), Color("red")])
                .lines(0..setpoint_adc_raw.len(),
                       setpoint_adc_raw,
                       &[Caption("setpoint [raw]"), Color("yellow")])
                .lines(0..compressor.len(),
                       compressor,
                       &[Caption("compressor"), Color("black")]);
