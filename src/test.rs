...
mod test {
    ...
    impl<'a> Adc for AdcSetpoint<'a> {
        fn read(&self) -> u32 {
            ...
            let a = &mut self.state.borrow_mut();
            a.setpoint = match ::rand::random::<u32>() % 100 {
    ...
    impl ::fridge::Tracer for Trace {
        fn trace(&mut self, data: &mut ::fridge::Data) {
        ...
        fn plot(&self, file: &str) {
            ...
            f.axes2d()
                ...
                .lines(0..setpoint_adc_raw.len(),
                       setpoint_adc_raw,
                       &[Caption("setpoint [raw]"), Color("purple")])
    ...
    fn run() {
        ...
        ::fridge::run(&p, &mut tracer, Some(700));

        tracer.plot("testrun.pdf");
    }
}
